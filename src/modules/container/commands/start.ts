...
export default defineCommand({
    name: 'start',
    description: 'Start a container with specified options',
    options: {
        hosts: {
            type: 'flag',
            alias: ['h'],
        ...
        names: {
            type: 'flag',
            alias: ['n'],
            description: 'List of container names to start',
            transform: (value) => value.split(',').map((v) => v.trim()),
        },
    },
    async execute({ options }) {
        ...
        for await (const host of hosts) {
            ...
            const command = `docker start ${containers.map((c) => c.name).join(' ')}`
