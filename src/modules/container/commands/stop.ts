...
export default defineCommand({
    name: 'stop',
    description: 'Stop a container with specified options',
    options: {
        hosts: {
            type: 'flag',
            alias: ['h'],
        ...
        names: {
            type: 'flag',
            alias: ['n'],
            description: 'List of container names to stop',
            transform: (value) => value.split(',').map((v) => v.trim()),
        },
    },
    async execute({ options }) {
        ...
        if (!options.hosts?.length && !options.names?.length) {
            const value = await confirm({
                message: 'Not defined filters will stop all containers. Do you want to continue?',
                default: false,
            })
        ...
        for await (const host of hosts) {
            ...
            const command = `docker stop ${containers.map((c) => c.name).join(' ')}`
